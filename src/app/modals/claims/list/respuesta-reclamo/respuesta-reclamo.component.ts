...
@Component({
  ...
  templateUrl: './respuesta-reclamo.component.html',
  styleUrls: ['./respuesta-reclamo.component.scss']
})
export class RespuestaReclamoComponent implements OnInit {
  ...
  getDetailReclamo() {
    this.apiService.getReclamoByIdService(this.dato.idreclamo, this.dato.empresa).then((response: any) => {
      ...
      this.resultado_reclamo = this.listDetalle[0]?.id_resultado_reclamo;
      this.motivo = this.listDetalle[0]?.id_motivo;
      this.listsarchivos = JSON.parse(this.listDetalle[0]?.r_archivos);
      // console.log('hora', this.listDetalle[0]?.d_solucion)
      if (this.listDetalle[0]?.d_solucion != "0000-00-00 00:00:00") {
        this.fechasolucion = this.datePipe.transform(this.listDetalle[0]?.d_solucion, 'yyyy-MM-dd');
        this.horasolucion = this.datePipe.transform(this.listDetalle[0]?.d_solucion, 'hh:mm');
        //console.log(this.fechasolucion, this.horasolucion)
      }
      if (this.listDetalle[0]?.d_respuesta_afi == '0000-00-00 00:00:00') {
        this.fecha_respuesta = "";
        this.fecha_respuesta_hora = "";
      } else {
        this.fecha_respuesta = this.datePipe.transform(this.listDetalle[0]?.d_respuesta_afi, 'yyyy-MM-dd');
        this.fecha_respuesta_hora = this.datePipe.transform(this.listDetalle[0]?.d_respuesta_afi, 'hh:mm');
      }
      if (this.listDetalle[0]?.id_estado_v2 === 2 || this.listDetalle[0]?.id_estado_v2 === 3) {
      ...
      if (this.listDetalle[0]?.id_estado_v2 === 4 || this.listDetalle[0]?.id_estado_v2 === 5) {
  ...
  getResultadoReclamo() {
    this.apiService.getListResultadoReclamoService().then((response: any) => {
  ...
  getListMedidas() {
    this.apiService.getListMedidasService(this.dato.idreclamo, this.dato.empresa).then((response: any) => {
  ...
  updateRespuestaReclamo() {
    ...
    const data = {
      ...
      fechasolucion: this.datePipe.transform(formValue.fechasolucion, 'yyyy-MM-dd') + ' ' + formValue.horasolucion,
      motivo: formValue.motivo,
      fecha_hora: this.datePipe.transform(formRespusta.fecha, 'yyyy-MM-dd') + ' ' + formRespusta.hora,
    ...
    this.showLoading();
    this.apiService.updateReclamoRespuestaService(data).then((response: any) => {
      Swal.close();
      this.getDetailReclamo();
      this.success()
    }, (error) => {
  ...
  openModalMedida(id, datos) {
    if (id === 1) {
      ...
      const modalRef = this.modalService.open(MedidasAdoptadasComponent, { size: 'xl' });
  ...
  deleteMedidas(id) {
    this.apiService.deleteMedidasService(id).then((response: any) => {
      Swal.close();
      this.success();
      this.getListMedidas();
    }, (error) => {
  ...
  getTipoReclamo(id) {
    var tiponame = "";
    if (id === 1) { tiponame = "Virtual" } else if (id === 2) { tiponame = "Físico" } else if (id === 3) { tiponame = "Telefónico" }
    return tiponame;
  }
  getNaturaleza(id) {
    var tiponame = "";
    if (id === 1) { tiponame = "Administrativa respecto a procesos propios de la IAFAS" } else if (id === 2) { tiponame = "Administrativa respecto a procesos propios de la IPRESS" } else if (id === 3) { tiponame = "Administrativa respecto a procesos propios de la UGIPRESS" }
    else if (id === 4) { tiponame = "Asistencial (Prestacional)" } else if (id === 5) { tiponame = "Administrativa y Asistencial (Prestacional)" }
    return tiponame;
  }

  getProceso(id) {
    var tiponame = "";
    if (id === 1) { tiponame = "Afiliación" } else if (id === 2) { tiponame = "Acreditación" } else if (id === 3) { tiponame = "Prestación de servicios salud" }
    else if (id === 4) { tiponame = "Liquidación, cobro o pago al usuario por los servicios prestados" } else if (id === 5) { tiponame = "Contratación de servicios de salud de una IAFAS a una IPRESS para atención de sus asegurados" }
    else if (id === 6) { tiponame = "Otro proceso" }
    return tiponame;
  }
