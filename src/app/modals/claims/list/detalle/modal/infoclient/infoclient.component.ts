...
export class InfoclientComponent implements OnInit {
  ...
  constructor(public activeModal: NgbActiveModal,
    private apiService: ClaimsService, private formBuilder: FormBuilder, private datePipe: DatePipe) {
  ...
  ngOnInit() {
    this.fechareclamo = this.datePipe.transform(this.datainfoclient.fechareclamo, 'yyyy-MM-dd');
    this.horareclamo = this.datePipe.transform(this.datainfoclient.fechareclamo, 'hh:mm'),
      this.fechareevento = this.datePipe.transform(this.datainfoclient.fechaevento, 'yyyy-MM-dd');
    this.horaevento = this.datePipe.transform(this.datainfoclient.fechaevento, 'hh:mm');

    this.fecharegistro = this.datePipe.transform(this.datainfoclient.fecharegistro, 'yyyy-MM-dd');
    this.horaregistro = this.datePipe.transform(this.datainfoclient.fecharegistro, 'hh:mm');
  ...
  validate() {
    ...
    if (this.datainfoclient.tipopaciente == 5 || this.datainfoclient.tipopaciente == 13 || this.datainfoclient.tipopaciente == 14 || this.datainfoclient.tipopaciente == 6 || this.datainfoclient.tipopaciente == 4) {
      this.showplansalud = false;
      this.showsegurosconvenios = true;
      this.showregiment = false;
      this.showafiliado = false;
    ...
    if (this.datainfoclient.tipopaciente == 22 || this.datainfoclient.tipopaciente == 21 || this.datainfoclient.tipopaciente == 3 || this.datainfoclient.tipopaciente == 22) {
      this.showplansalud = false;
      this.showsegurosconvenios = false;
      this.showregiment = true;
      this.showafiliado = false;
  ...
  getConveniosSeguros(id) {
    //console.log(id);
    this.apiService.getConveniosSeguroService(this.datainfoclient.tipopaciente, id).then((response: any) => {
  ...
  getProvincia(departamento) {
    this.apiService.getProvinciaService(departamento).then((response: any) => {
      ...
      this.getDistrito(this.datainfoclient.departamento.trim(), this.datainfoclient.provincia.trim());
    });
  }
  ...
  updateInfoClient() {
    ...
    const data = {
      ...
      fechareclamo: this.datePipe.transform(fomrValue.fechareclamo, 'yyyy-MM-dd') + ' ' + fomrValue.horareclamo,
      ...
      fechaevento: this.datePipe.transform(fomrValue.fechaevento, 'yyyy-MM-dd') + ' ' + fomrValue.hotaevento,
      // fechasolucion: this.datePipe.transform(fomrValue.fechasolucion, 'yyyy-MM-dd') + ' ' + fomrValue.horasolucion,
      fecharegistro: this.datePipe.transform(fomrValue.fecharegistro, 'yyyy-MM-dd') + ' ' + fomrValue.horaregistro,
    ...
    this.showLoading();
    this.apiService.updateReclamoInfoClientService(data).then((response: any) => {
