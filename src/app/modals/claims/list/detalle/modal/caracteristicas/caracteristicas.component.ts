...
export class CaracteristicasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.name_causa_especifica = this.dataCaracteristicas.name_causa_especifica;
    if (this.dataCaracteristicas.envio_csalud === null || this.dataCaracteristicas.envio_csalud === '') {
  ...
  getNivelReclamo() {
    this.apiService.getNivelReclamoService().then((response: any) => {
      this.listnevereclamo = response.data.length > 0 ? response.data : [];
    });
  }
  ...
  updateCaracteriscas() {
    ...
    this.showLoading();
    this.apiService.updateReclamoCaracteristicasService(data).then((response: any) => {
      const data = {
        success: 1
      }
