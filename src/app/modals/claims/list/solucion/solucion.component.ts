...
export class SolucionComponent implements OnInit {
  ...
  getDetailReclamo() {
    this.apiService.getReclamoByIdService(this.dato.idreclamo, this.dato.empresa).then((response: any) => {
      ...
      this.codigo_de_reclamo = this.listDetalle[0]?.v_cod_inst + this.datePipe.transform(this.listDetalle[0]?.d_fecha_reclamo, 'dMyyy') + this.listDetalle[0]?.v_correlativo_reclamo + '-' + this.listDetalle[0]?.v_correlativo_causa;
      this.accionestomadas = this.listDetalle[0]?.v_acciones_tomadas;
      this.accionestomadas_usuario = this.listDetalle[0]?.v_acciones_tomadas_usuario;
      this.observacion = this.listDetalle[0]?.v_obs_interno;
      this.calificacionejecutiva = this.listDetalle[0]?.id_calificacion_ejecutiva;
      ...
      if (this.listDetalle[0]?.v_cod_medico != 0) {
        ...
        this.resultpaterno = this.listDetalle[0]?.v_apellido_p_medico;
        this.resultmaterno = this.listDetalle[0]?.v_apellido_m_medico;
      } else if (this.listDetalle[0]?.c_afiliado3 != "" || this.listDetalle[0]?.c_afiliado3 != "0") {
  ...
  getDepartament() {
    this.apiService.getDepartamentoByIdService(this.listDetalle[0]?.id_departamento.trim()).then((response: any) => {
      this.listdepartamentos = response
      if (this.listdepartamentos == null || this.listdepartamentos == '') {
        this.namedepartamento = "";
      } else {
        this.namedepartamento = this.listdepartamentos.DescripcionCorta;
      }
  ...
  getPrivincia() {
    this.apiService.getProvinciaByIdService(this.listDetalle[0]?.id_departamento.trim(), this.listDetalle[0]?.id_provincia.trim()).then((response: any) => {
  ...
  getDistritto() {
    this.apiService.getDistritoByIdService(this.listDetalle[0]?.id_departamento.trim(), this.listDetalle[0]?.id_provincia.trim(), this.listDetalle[0]?.id_distrito.trim()).then((response: any) => {
  ...
  getParentesco() {
    this.apiService.getParentescoByIdService(this.listDetalle[0]?.id_parentesco).then((response: any) => {
      this.parentesco = response.data.length > 0 ? response.data : [];
    });
  }
  getModoRecepcion() {
    this.apiService.getRecepcionByIdService(this.listDetalle[0]?.id_modorecepcion_v2).then((response: any) => {
  ...
  getReclamo() {
    ...
    this.apiService.getReclamoDetailService(data).then((response: any) => {
      this.listdetailreclamo = response.data.length > 0 ? response.data : [];
    });
  }
  ...
  getCodigoLibroService() {
    this.apiService.getCodigoLibroService(this.dato.idreclamo, this.dato.empresa).then((response: any) => {
      this.listcodigolibro = response.data.length > 0 ? response.data : [];

    })
  }
  getCalificacionEjecutiva() {
    this.apiService.getCalificacionEjecutiva().then((response: any) => {
      this.calificationsjecutiva = response.data.length > 0 ? response.data : [];
    });
  }
  ...
  updateSolcuion() {
    ...
    this.showLoading();
    this.apiService.updateReclamoSolutionsService(data).then((response: any) => {
      Swal.close();
      this.getDetailReclamo();
      this.textaccion = "";
      this.textobs = "";
  ...
  addCommentary() {
    ...
    if (form.obs.trim() === '') {
      ...
      this.detalleobservacion_usuario = '/ ' + this.datePipe.transform(new Date(), 'yyyy-MM-dd') + ' ' + this.datePipe.transform(new Date(), 'h:mm:ss') + ': Usuario (' + this.username + ' ' + this.firstname + ') ' + form.obs;
    }
  ...
  addObservation() {
    ...
    if (form.detalle_eboservacion.trim() === '') {
      ...
      this.detalle_obs = this.datePipe.transform(new Date(), 'yyyy-MM-dd') + ' ' + this.datePipe.transform(new Date(), 'h:mm:ss') + ': Usuario (' + this.username + ' ' + this.firstname + ') ' + form.detalle_eboservacion;
    }
  }
  //NODAL
  open(id) {
    if (id == 1) {
      ...
      const modalRef = this.modalService.open(InfoclientComponent, { size: 'lg' });
    ...
    if (id == 2) {
      ...
      const modalRef = this.modalService.open(RepresentanteComponent, { size: 'lg' });
    ...
    if (id == 3) {
      ...
      const modalRef = this.modalService.open(DetalleInvolucradoComponent, { size: 'lg' });
    ...
    if (id == 4) {
      const data = {
        ...
        name_causa_especifica: this.listDetalle[0]?.name_causa_especifica,
      ...
      const modalRef = this.modalService.open(CaracteristicasComponent, { size: 'lg' });
  ...
  openDocs(url) {
    ...
    const modalRef = this.modalService.open(DocsComponent, { size: 'xl' });
