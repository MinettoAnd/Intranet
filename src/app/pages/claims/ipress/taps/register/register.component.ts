...
export class RegisterIpresComponent implements OnInit {
  ...
  public codigo_institucion: string = "299999999"
  ...
  constructor(private modalService: NgbModal
    ...
    , private apiService: ClaimsService, private config: NgSelectConfig,
  ...
  open() {
    ...
    const data = {
      ...
      txtsearch: this.f.txtsearch.value,
    };
    this.loading('Realizando Busqueda ...')
    this.apiService.searchPerson(data).then((response: any) => {
      this.listSearchPerson = response.length > 0 ? response : [];
      Swal.close();
      if (this.listSearchPerson.length >= 1) {

        const modalRef = this.modalService.open(SearchComponent, { size: 'lg' });
        modalRef.componentInstance.datasearsh = this.listSearchPerson;
  ...
  getTypeDocument() {
    this.apiService.getTipoDocumentService().then((response: any) => {
      this.listtypedocument = response.data.length > 0 ? response.data : [];
  ...
  getProvincia(departamento) {
    this.apiService.getProvinciaService(departamento).then((response: any) => {
  ...
  getDistrito(departamento, provincia) {
    this.apiService.getDistritoService(departamento, provincia).then((response: any) => {
  ...
  showEmpresas(event) {
    this.name_tipo_paciente = event.target.options[event.target.options.selectedIndex].text;
    ...
    } else if (event.target.value === '5' || event.target.value === '13' || event.target.value === '14' || event.target.value === '6' || event.target.value === '4') {
    ...
    } else if (event.target.value === '22' || event.target.value === '21' || event.target.value === '3' || event.target.value === '23') {
      this.showplansalud = false;
      this.showsegurosconvenios = false;
  ...
  //get plan salud
  getPlanSalud() {
    this.apiService.getPlanSaludTipoPacienteService().then((response: any) => {
      this.listplansalud = response.length > 0 ? response : [];
    });
  }
  getByIdConveniosSeguros(id) {
    this.apiService.getConveniosSeguroByIdService(id).then((response: any) => {
  ...
  getSucursalAsignado() {
    this.apiService.getSurcursalAsignado(localStorage.getItem('username')).then((response: any) => {
  ...
  getEstadoTramite() {
    this.apiService.getEstadoTramiteService().then((response: any) => {
      this.listestadotramites = response.data.length > 0 ? response.data : [];
      // this.getGarantiaSalud();
    })
  }
  onChangeEstado(event) {
    //console.log(event.target.options[event.target.options.selectedIndex].text);
    this.nameestado = event.target.options[event.target.options.selectedIndex].text;
    // console.log(this.name_estado);
  }
  onChangeRegmin(event) {
    //console.log(event.target.options[event.target.options.selectedIndex].text);
    this.nameregimen = event.target.options[event.target.options.selectedIndex].text;
    // console.log(this.name_estado);
  }
  ...
  onChangeCausas(event) {
    this.name_garantia = event.target.options[event.target.options.selectedIndex].text;
  ...
  getCausasEspecificas(id: number, data) {
    switch (id) {
      case 1:
        this.apiService.getCausaEspecifica(data).then((response: any) => {
          this.listcuasasespeciales = response.data.length > 0 ? response.data : [];
        })
        break;
      case 2:
        this.apiService.getCausaEspecifica(data).then((response: any) => {
          this.listcuasasespeciales2 = response.data.length > 0 ? response.data : [];
        })
        break;
      case 3:
        this.apiService.getCausaEspecifica(data).then((response: any) => {
          this.listcuasasespeciales3 = response.data.length > 0 ? response.data : [];
        })
        break;
      default:
  ...
  onChangeCausasDetail(id: number, event) {
    this.name_causa_especifica = event.target.options[event.target.options.selectedIndex].text;
  ...
  getDetailCausaEspecifica(id, event) {
    switch (id) {
      case 1:
        this.apiService.getDetailCausaEspecifica(event).then((response: any) => {
          this.listdetailcausa = response.data.length > 0 ? response.data : [];
          this.fechalimite = this.sumarDias(new Date(), this.listdetailcausa[0]?.n_tiempo_max)

        })
        break;
      case 2:
        this.apiService.getDetailCausaEspecifica(event).then((response: any) => {
          this.listdetailcausa2 = response.data.length > 0 ? response.data : [];
          this.fechalimite2 = this.sumarDias(new Date(), this.listdetailcausa2[0]?.n_tiempo_max)

        })
        break;
      case 3:
        this.apiService.getDetailCausaEspecifica(event).then((response: any) => {
          this.listdetailcausa3 = response.data.length > 0 ? response.data : [];
          this.fechalimite3 = this.sumarDias(new Date(), this.listdetailcausa3[0]?.n_tiempo_max)
  ...
  //onchange personal involucrado isTrusted
  ...
  searchPeraConocimiento(id) {
    if (id == 1) {
      ...
      this.apiService.searchInvolucradosService(data).then((response: any) => {
        ...
        modalRef.componentInstance.datapersonal01 = this.listpersobals1;
        modalRef.result.then((result) => {
          if (result.cod === 1) {
    ...
    if (id == 2) {
      ...
      this.apiService.searchInvolucradosService(data).then((response: any) => {
        ...
        Swal.close();
        const modalRef = this.modalService.open(ConocimientoComponent, { size: 'lg' });
        modalRef.componentInstance.datapersonal02 = this.listpersobals2;
        modalRef.result.then((result) => {
          if (result.cod === 2) {
    ...
    if (id == 3) {
      ...
      this.apiService.searchInvolucradosService(data).then((response: any) => {
        ...
        Swal.close();
        const modalRef = this.modalService.open(ConocimientoComponent, { size: 'lg' });
        modalRef.componentInstance.datapersonal03 = this.listpersobals3;
        modalRef.result.then((result) => {
          if (result.cod === 3) {
  ...
  showConveniosModal(data) {
    const modalRef = this.modalService.open(ConveniosComponent, { size: 'xl' });
    modalRef.componentInstance.dato = data;
    modalRef.result.then((result) => {      //resultado
      // console.log(result);
      this.rucconveios = '';
  ...
  registroReclamo() {
    ...
    switch (parseInt(detalle.sede)) {
      case 1:
        this.cod_ugipress = '00008281';
        this.cod_admi_d = '00008281';
        this.cod_admi_p = '00008281';
        break;
      case 2:
        this.cod_ugipress = '00010251';
        this.cod_admi_d = '00010251';
        this.cod_admi_p = '00010251';
        break;
      case 4:
        this.cod_ugipress = '00015118';
        this.cod_admi_d = '00015118';
        this.cod_admi_p = '00015118';
        break;
    ...
    this.nombrecomplepaciente = paciente.nombre + ' ' + paciente.paterno + ' ' + paciente.materno;
    if (acciones.p1code == '') {
    ...
    this.apiService.getMaxIdLibroService(data).then((response: any) => {
      if (response.success === 1) {
        ...
        this.max_id_libro = ('000000' + this.maxids[0]?.maximo).slice(-6);
        ...
        const data = {
          ...
          fecha_traslado: this.datePipe.transform(detalle.fechaevento, 'yyyy-MM-dd') + ' ' + detalle.horarenvento,
          ...
          acciones_tomadas_usuario: this.datePipe.transform(new Date(), 'yyyy-MM-dd') + ' ' + this.datePipe.transform(new Date(), 'h:mm:ss') + ': Usuario (' + this.inicialnombre + ' ' + this.inicialapellido + ')' + ' ' + acciones.descripcionscciones,
          ...
          nombre_completo: paciente.nombre + ' ' + paciente.paterno + ' ' + paciente.materno,
          ...
          d_registro_reclamo: this.datePipe.transform(detalle.fechareclamo, 'yyyy-MM-dd') + ' ' + detalle.horareclamo,
          d_fecha_reclamo: this.datePipe.transform(detalle.fechaevento, 'yyyy-MM-dd') + ' ' + detalle.horarenvento,
          d_registro: this.datePipe.transform(new Date(), 'yyyy-MM-dd') + ' ' + this.datePipe.transform(new Date(), 'h:mm:ss'),
          ...
          n_tipo_reclamo: '1',
          cod_otra_persona_involucrado: involucrado.codinvolucrado,//CODIGO DEL PERSONAL INVOLUCRADO
          ...
          apellido_para_conocimiento: acciones.p1paterno + ' ' + acciones.p1materno,
          nombre_para_conocimiento2: acciones.p2nombre,
          apellido_para_conocimiento2: acciones.p2paterno + ' ' + acciones.p2materno,
          nombre_para_conocimiento3: acciones.p3nombre,
          apellido_para_conocimiento3: acciones.p3paterno + ' ' + acciones.p3materno,
          cod_afiliado: paciente.codigo_afiliado,
          d_respuesta_limite: '',
          empresa: this.TIPO_RECLAMO
        }
        //console.log(data);
        this.apiService.postReclamoService(data).then((response: any) => {
          if (response.success === 1) {
            console.log('RECLAMO REGISTRADO  exitoso');
            this.listrespreclamo = response.data.length > 0 ? response.data : [];
            this.maxid_reclamo = this.listrespreclamo[0]?.id_reclamo;
            this.getSedesById(this.m_sede, this.maxid_reclamo);
          ...
          Swal.close();
          this.showErro('Problemas al generar el reclamo', 'error presentado:' + error);
        });
      }
    })
  ...
  getSedesById(sede, idmax_reclamo) {
    this.apiService.getSedeByIDService(sede).then((response: any) => {
      ...
      this.vcorrelativo = "HP" + this.byidsede + ('000000' + this.maxids[0]?.maximo).slice(-6);
      this.newAudita();
      this.registerCorrelativoLibro('000000' + this.maxids[0]?.maximo, this.vcorrelativo, idmax_reclamo, sede)
    }, (error) => {
      Swal.close();
      this.showErro('Problamas al optener el id de la sede', 'error presentado:' + error);
    });
  }
  ...
  /* getMaxIdLibro(sede, idmax_reclamo) {
     ...
     this.apiService.getMaxIdLibroService(data).then((response: any) => {
       ...
       this.vcorrelativo = "HP" + this.byidsede + (this.max_id_libro).slice(-6);
  ...
  /*registerCorrelativo() {
    ...
    this.apiService.postCorrelativoService(data).then((response: any) => {
  ...
  //ruc web
  openRuc() {
    window.open('https://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/FrameCriterioBusquedaMovil.jsp', 'targetWindow', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,'); return false;
  }
