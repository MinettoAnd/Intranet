...
@Component({
  selector: 'app-list-ipres',
  templateUrl: './list.component.html',
  styleUrls: ['./list.component.scss']
})
export class ListIpresComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private formBuilder: FormBuilder,
    private apiService: ClaimsService, private modalService: NgbModal) {
  ...
  searhReclamo() {
    ...
    const data = {
      ...
      fechainicio: this.datePipe.transform(form.fechainicio, 'yyyy-MM-dd'),
    ...
    this.apiService.getReclamoListService(data).then((response: any) => {
      this.listReclamo = response.data.length > 0 ? response.data : [];
  ...
  searhMedidasAdoptadas() {
    ...
    this.apiService.getSearchMedidasAdoptadas(data).then((response: any) => {
  ...
  exportExcel() {
    ...
    onlyNameAndSymbolArr = this.listReclamo.map(x => ({
      'N° Reclamo': parseInt(x.cod_administrativo_d) + '-' + this.newCorrelativoCorrecto(x.idcorrelativo),
      ...
      'Afectado': x.V_nombre1.trim() + ' ' + x.v_paterno1.trim() + ' ' + x.v_materno1.trim(),
  ...
  exportExcelTrama1() {
    ...
    if (form.sede === 0) {
      return Swal.fire('Info...', 'Seleccione la sede del cual desear generar la trama para la descarga', 'info');
    }
    ...
    onlyNameAndSymbolArr = this.listReclamo.map(x => ({
      ...
      codigo_administrativo_presento_reclamo: parseInt(x.cod_administrativo_p),//falta difinir
      medio_presentacion_reclamo: x.id_modorecepcion_v2,
      codigo_unico_reclamo: parseInt(x.cod_administrativo_d) + '-' + this.newCorrelativoCorrecto(x.idcorrelativo),// + '-1',
      ...
      tipo_documento_presenta_reclamo: parseInt(this.getDoc(x.n_tipo_doc_titular, x.n_tipo_doc_reclama.trim())),
      numere_documento_presenta_reclamo: this.getName(x.n_tipo_doc_reclama.trim(), x.v_doc_titular.trim(), x.v_doc_reclama.trim()),
      razon_social2: x.razon_social_r,
      nombres2: this.getName(x.n_tipo_doc_reclama.trim(), x.V_nombre1.trim(), x.v_Nombres2.trim()),
      apellido_paterno2: this.getName(x.n_tipo_doc_reclama.trim(), x.v_paterno1.trim(), x.v_paterno2.trim()),
      apellido_materno2: this.getName(x.n_tipo_doc_reclama.trim(), x.v_materno1.trim(), x.v_materno2),
    ...
    TableUtil.exportArrayToExcel(onlyNameAndSymbolArr, this.getNameFileTramite());
  }
  exportExcelTrama2() {
    const form = this.formSearchReclamo.value;
    if (form.sede === 0) {
    ...
    TableUtil.exportArrayToExcel(onlyNameAndSymbolArr2, this.getNametxtTramite());
  }
  ...
  exportTramaTXT_01() {
    ...
    new Angular2Txt(onlyNameAndSymbolArr, this.getNameFileTramite(), this.options);
  ...
  exportTramaTXT_02() {
    ...
    new Angular2Txt(onlyNameAndSymbolArr, this.getNametxtTramite(), this.options);
  ...
  open(option, idreclamo, sede, correlativo) {
    ...
    if (option === 2) {
      ...
      const modalRef = this.modalService.open(SolucionComponent, { size: 'xl' });
      modalRef.componentInstance.dato = data;
    }
    if (option === 3) {
      const data = {
      ...
      const modalRef = this.modalService.open(ListcomocimientoComponent, { size: 'xl' });
      modalRef.componentInstance.dato = data;
    }
    if (option === 4) {
      const data = {
      ...
      const modalRef = this.modalService.open(RespuestaReclamoComponent, { size: 'xl' });
      modalRef.componentInstance.dato = data;
    }
  ...
  getNameFileTramite() {
    ...
    if (parseInt(form.sede) === 1) {
      return 8281 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'RECLAMOS';
    } else if (parseInt(form.sede) === 2) {
      return 10251 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'RECLAMOS';
    } else if (parseInt(form.sede) === 4) {
      return 15118 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'RECLAMOS';
    }
  }
  ...
  getNametxtTramite() {
    const form = this.formSearchReclamo.value;
    if (parseInt(form.sede) === 1) {
      return 8281 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'MEDIDAS';
    } else if (parseInt(form.sede) === 2) {
      return 10251 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'MEDIDAS';
    } else if (parseInt(form.sede) === 4) {
      return 15118 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'MEDIDAS';
    }
  }
