...
export class ListComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private formBuilder: FormBuilder, private apiService: ClaimsService, private modalService: NgbModal) {
  ...
  exportExcel() {
    ...
    onlyNameAndSymbolArr = this.listReclamo.map(x => ({
      'N° Reclamo': parseInt(x.cod_administrativo_d) + '-' + this.newCorrelativoCorrecto(x.v_correlativo_reclamo.trim()),
  ...
  exportExcelTrama1() {
    ...
    onlyNameAndSymbolArr = this.listReclamo.map(x => ({
      ...
      codigo_unico_reclamo: parseInt(x.cod_administrativo_d) + '-' + parseInt(x.v_correlativo_reclamo),
  ...
  exportTramaTXT_01() {
    ...
    onlyNameAndSymbolArr = this.listReclamo.map(x => ({
      ...
      fecha_presentada_reclamo: this.diaSolucion(x.d_fecha_reclamo),
      detalle_reclamo: x.v_descripcion_reclamo.trim(),
      servicio_origen: ('0'+x.id_servicio_efectuado).substring(('0'+x.id_servicio_efectuado).length-3,2),//verficar
      competencia_para_atencion: x.id_competencia_atencion,//verficar
      causa_del_reclamo_01: x.id_maestro_causa_especifica,
      causa_del_reclamo_02: x.id_maestro_causa_especifica_2 == null ? '' :x.id_maestro_causa_especifica_2,
      causa_del_reclamo_03: x.id_maestro_causa_especifica_3 == null ? '' :x.id_maestro_causa_especifica_3,
      estado_reclamo: x.id_estado_v2,
      codigo_reclamo_primigenio: x.codigo_reclamo,
      etapa_reclamo: x.estapa_reclamo,
      tipo_administrador: x.tipo_admin,
...
console.log(onlyNameAndSymbolArr);
  ...
  exportTramaTXT_02() {
    ...
    new Angular2Txt(onlyNameAndSymbolArr2, this.getNametxtTramite(), this.options);
  ...
  open(option, idreclamo, sede, correlativo) {
    ...
    if (option === 3) {
      ...
      const modalRef = this.modalService.open(ListcomocimientoComponent, { size: 'lg' });
      modalRef.componentInstance.dato = data;
    }
    if (option === 4) {
      const data = {
  ...
  getNameFileTramite() {
    const form = this.formSearchReclamo.value;
    return 20027 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'RECLAMOS';
  }

  getNametxtTramite() {
    const form = this.formSearchReclamo.value;
    return 20027 + '_' + this.datePipe.transform(form.fechafin, 'yyyy') + '_' + this.datePipe.transform(form.fechafin, 'MM') + '_' + 'MEDIDAS';
