...
export class RegisterComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal
    ...
    , private apiService: ClaimsService, private datePipe: DatePipe) {
  ...
  registroReclamo() {
    ...
    this.apiService.getCorrelativoService(this.TIPO_RECLAMO).then((response: any) => {
      if (response.success === 1) {
        this.listcorretalivos = response.data.length > 0 ? response.data : [];
        this.numcorrelativo = ("000000" + this.listcorretalivos[0]?.CORRELATIVO).slice(-6);
