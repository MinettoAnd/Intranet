...
@Component({
  selector: 'app-content-inventario',
  templateUrl: './content-inventario.component.html',
  styleUrls: ['./content-inventario.component.scss']
})
export class ContentInventarioComponent implements OnInit {
  isTable0 = true;
  isTable1 = true;
  isTable2 = true;
  isTable3 = true;
...
updateFilter(event) {
  ...
  if(numberTabla === 1){
    this.rows1.map(item=>{
      item.StockActual = typeof item.StockActual === 'number' ? item.StockActual : Number(item.StockActual);
