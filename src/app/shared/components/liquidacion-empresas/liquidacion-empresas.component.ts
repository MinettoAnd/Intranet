...
import { Component, ElementRef, OnInit, PipeTransform, ViewChild, ViewEncapsulation,Input } from '@angular/core';
...
@Component({
  selector: 'app-liquidacion-empresas',
  templateUrl: './liquidacion-empresas.component.html',
  styleUrls: ['./liquidacion-empresas.component.scss']
})
export class LiquidacionEmpresasComponent implements OnInit {
  active = 1;
  closeResult = '';
  ...
  innerHtml: any;
  empresaMonto: any;
  empresaMontoTotal: any;
  @Input() area:string;
  constructor(private tableApiservice: LaboratorioService, private exportService: ExportService,
    private _cp: CurrencyPipe, private _ccp: CustomCurrencyPipe, private _ndp:NumberDecimalPipe, private _dp: DecimalPipe, private _pp:PorcentajePipe, private _cnp:CustomNumberPipe, private modalService: NgbModal) { 
              ...
              this.tableApiservice.getPagoEntreEmpresa(this.parameters).subscribe(
                (response) => { 
                  ...
                  //   this.rows3filtered = this.rows3.filter(item => item.GRUPOEM === 'CANTIDAD');
                    ...
                    this.empresaMonto = this.rows6[1].MONTO
                    this.empresaMontoTotal = this.rows6[1].TOTAL
                    console.log(this.rows6[1])
