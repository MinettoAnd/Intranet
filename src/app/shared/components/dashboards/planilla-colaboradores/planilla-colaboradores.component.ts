...
@Component({
  selector: 'app-planilla-colaboradores',
  templateUrl: './planilla-colaboradores.component.html',
  styleUrls: ['./planilla-colaboradores.component.scss']
})
export class PlanillaColaboradoresComponent implements OnInit {
  initialSize = 0;
  active = 1;
  enableSummary = true;
 ...
 private getDismissReason(reason: any): string {
  ...
  grupoChange(event){
    ...
    this.grupo = input;
    if (input.length > 0) {
      const filtered = this.rows3filtered = this.rows9.filter(item => item.grupo === input);
  ...
  especialityChange(event){
    ...
    if (input.length > 0 && input !== 'null') {
      const filtered = this.rows4filtered = this.rows10.filter(item => item.especialidad === input);
        // console.log(filtered);
      this.rows4filtered = [...filtered]
      
     } else {

      this.rows4filtered = [...this.rows10]
    }
  }
  ...
  updateFilter(event, selectedOption) {
    ...
    if (input.length > 0) {
      ...
      this.rows3filtered = [...this.rows9.filter(item => item.grupo === selectedOption)]
    }
  }
  updateFilter2(event, selectedOption) {
    ...
    if (input.length > 0) {
      ...
      this.rows4filtered = [...this.rows10.filter(item => item.especialidad === selectedOption)]
    }
  }
